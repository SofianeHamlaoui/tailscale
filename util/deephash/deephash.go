...
func typeIsRecursive(t reflect.Type) bool {
	...
	visitType = func(t reflect.Type) (isRecursiveSoFar bool) {
		...
		case reflect.Interface:
			...
			// it for now.
			if debug {
				log.Printf("recursive type: %v due to interface", t)
				for i, t := range stack {
					log.Printf("%v: %v", i, t)
				}
			}
...
func (h *hasher) hashValue(v reflect.Value, forceCycleChecking bool) {
	...
	checkCycles := forceCycleChecking || ti.isRecursive
	...
	case reflect.Ptr:
		...
		h.hashUint8(1) // indicates visiting a pointer
		h.hashValue(v.Elem(), checkCycles)
	case reflect.Struct:
		for i, n := 0, v.NumField(); i < n; i++ {
			h.hashValue(v.Field(i), checkCycles)
		}
	case reflect.Slice, reflect.Array:
		...
		for i := 0; i < vLen; i++ {
			...
			h.hashValue(v.Index(i), checkCycles)
		}
	case reflect.Interface:
		if v.IsNil() {
			h.hashUint8(0) // indicates nil
...
func (h *hasher) hashMap(v reflect.Value, checkCycles bool) {
	...
	for iter.Next() {
		...
		e.SetIterValue(iter)
		mh.h.reset()
		mh.h.hashValue(k, checkCycles)
		mh.h.hashValue(e, checkCycles)
		sum.xor(mh.h.sum())
	}
